...
void doublerCapaciteNouveauTableau(ListeFilms& tableauFilms, Film* nouvFilm) { 
	Film** nouveauTableau = new Film * [tableauFilms.capacite];

	if (tableauFilms.capacite == 0) {
		nouveauTableau[tableauFilms.capacite++]; 
	}

	else if (tableauFilms.capacite <= tableauFilms.nElements) {
		nouveauTableau[tableauFilms.capacite * 2]; 
		
		for (int i = 0; i <= tableauFilms.nElements; i++) {
			nouveauTableau[i] = tableauFilms.elements[i];
		}
	}
	tableauFilms.elements = nouveauTableau;
	nouveauTableau[tableauFilms.nElements] = nouvFilm;
	tableauFilms.nElements++;
	delete[] &tableauFilms;
}
...
void effacerPtrFilm(ListeFilms& tableauFilms, Film* filmAEnlever) {
	for (int j = 0; j <= tableauFilms.nElements; j++) {
		if (tableauFilms.elements[j] == filmAEnlever) {
			delete tableauFilms.elements[j];
			tableauFilms.nElements - 1; 
		}
		else {
		}
	}
...
Acteur* trouverActeurDansListeFilm(const ListeFilms& tableauFilms, string nomActeurDesire) {
	Acteur* acteurVoulu = nullptr;
	for (const Film* ptrFilm : span(tableauFilms.elements, tableauFilms.nElements)) {
...
Acteur* lireActeur(istream& fichier)
{
	...
	return {}; //TODO: Retourner un pointeur soit vers un acteur existant ou un nouvel acteur ayant les bonnes informations, selon si l'acteur existait déjà.  
